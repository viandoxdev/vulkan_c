...
#define _VECTOR__IMPL(T, V, qualifier) \
    ...
    __attribute__((unused)) static inline V _vec_##qualifier##_init() { return (V)vec_init(); } \
    ...
    __attribute__((unused)) static inline void _vec_##qualifier##_push_array(V *vec, T *vals, size_t count) { \
    ...
    __attribute__((unused)) static inline void _vec_##qualifier##_clear(V *vec) { vec->len = 0; } \
...
#define _VECTOR_GENERIC(expr, x) \
    _Generic(expr, EVAL(CALL(_VECTOR_MAP_ADD, _VECTOR_GEN, _##x, VECTOR_IMPL_LIST)) struct _VectorNever: (void)0)
